...
Ext.ux.BufferedList = Ext.extend(Ext.List, {
    ...
    refresh: function() {
        ...
        if(store.currentPage >= 2){
            if(store.clearOnPageLoad === false ){
                this.renderOnScroll(undefined, true);    
                return
            }else{
    ...
    renderOnScroll: function(startRecord,isCalledNextPage) { // startRecord optional
        ...
        if ( Ext.isNumber(startRecord) ) {
            ...
            if(isCalledNextPage === true){
                console.log('nextPage is buflit')
                newTop = previousTop;
                newBottom = Math.min(previousBottom + this.batchSize,maxIndex);
                scrollDown = true;
                incrementalRender = true;
